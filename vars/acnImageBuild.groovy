...
def call(body) {
    ...
    dir("${directory}/ocp-artifact-${imageType}") {
        if ( imageType == "application" ) {
            ...
            sh "sed -i \"s~#APP_PROTOCOL#~${GLOBAL_VARS['APP_PROTOCOL']}~g\" ${directory}/pipeline/dockerfiles/${appLang}/dockerfiles/application/Dockerfile"
            sh "sed -i \"s~#APP_PORT#~${GLOBAL_VARS['APP_PORT']}~g\" ${directory}/pipeline/dockerfiles/${appLang}/dockerfiles/application/Dockerfile"
    ...
    container(name: 'jnlp'){
        sh "oc apply -f ${directory}/pipeline/openshift-artifacts/${openshiftVersionFolder}/${imageType}/buildconfig-docker-image-from-dir.yaml -n ${namespace_cicd}"
        sh "oc start-build ${nameBuildconfig} --from-dir=${directory}/ocp-artifact-${imageType}/ --follow -n ${namespace_cicd}"
    }
